...
import { IsImportedWallet, WALLET_TYPE_HDWALLET, WALLET_TYPE_IMPORTED, isImportedAccount } from "../../storage/types";
...
export class WalletApi implements IWalletApi {
	...
	async createWallet(params: CreateWalletParams): Promise<Wallet> {
		...
		const t = Date.now();
		const vault = await Vault.fromEncryptedMnemonic(hdPath, token, wallet.encryptedMnemonic);
	...
	async checkMnemonicDuplicated(mnemonic: string, token: Buffer): Promise<boolean> {
		const wallets = await this.storage.getWallets();
		for (const wallet of wallets) {
			if (IsImportedWallet(wallet)) {
				continue;
			}
			const decryptedMnemonic = crypto.decryptMnemonic(token, wallet.encryptedMnemonic!);
	...
	async importWallet(params: ImportWalletParams): Promise<Wallet> {
		...
		let privateKeyPair;
		try {
			privateKeyPair = new elliptic.eddsa("ed25519").keyFromSecret(Buffer.from(privateKeyArrayBuffer));
		} catch (e) {
		...
		const t = Date.now();
		const encryptedPrivateKey = crypto.encryptPrivate(token, privateKeyArrayBuffer).toString("hex");
		const vault = await Vault.fromEncryptedPrivateKey(token, encryptedPrivateKey);
		console.log("vault create time", Date.now() - t);
		// TODO: cache vaults
		const account = {
			id: accountIdStr,
	...
	async checkPrivateDuplicated(privateKey: elliptic.eddsa.KeyPair, token: Buffer): Promise<boolean> {
		const wallets = await this.storage.getWallets();
		for (const wallet of wallets) {
			if (IsImportedWallet(wallet)) {
				for (const account of wallet.accounts) {
					...
					if (!accountData) {
						throw new Error(`Data inconsistency: wallet ${wallet.id} account ${account.id} not exist`);
					}
					if (!isImportedAccount(accountData)) {
						throw new Error(`Data inconsistency: account ${account.id} is not imported`);
					}
					const decryptedPrivateKey = crypto.decryptPrivate(token, accountData.encryptedPrivateKey!);
					if (decryptedPrivateKey.getSecret("hex") === privateKey.getSecret("hex")) {
						return true;
					}
				}
			} else {
				for (const account of wallet.accounts) {
					...
					if (isImportedAccount(accountData)) {
						throw new Error(`Data inconsistency: account ${account.id} is imported`);
					}
					...
					if (vault.getPrivateKey().toString("hex") === privateKey.getSecret("hex")) {
						return true;
					}
				}
			}
