...
export function decryptPrivate(token: Buffer, encryptedPrivate: string): elliptic.eddsa.KeyPair {
	const aesCtr = new ModeOfOperation.ctr(token);
	const encryptedBytes = Buffer.from(encryptedPrivate, "hex");
	const privateBytes = aesCtr.decrypt(encryptedBytes);
	let keyPair;
	try {
		keyPair = new elliptic.eddsa("ed25519").keyFromSecret(Buffer.from(privateBytes));
	} catch (e) {
		throw new Error("Invalid password");
	}
	return keyPair;
...
export function password2Token(password: string, salt: Buffer) {
	...
	return Buffer.from(Pbkdf2HmacSha256(hash, salt, PBKDF2_NUM_OF_ITERATIONS, PBKDF2_KEY_LENGTH));
}
