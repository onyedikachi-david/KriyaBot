...
export class Provider {
	...
	constructor(queryEndpoint: string, txEndpoint: string, versionCacheTimoutInSeconds: number) {
	...
	async getTransferCoinTxb(coinType: string, amount: bigint, recipient: string, address: string) {
	 ...
	 * @param vault
	 */
	async transferCoin(coinType: string, amount: bigint, recipient: string, vault: Vault) {
		...
		return await this.tx.transferCoin(coins, coinType, amount, recipient, vault);
	}

	async transferObject(objectId: string, recipient: string, vault: Vault, gasBudget?: number) {
		const object = await this.query.getOwnedObject(vault.getAddress(), objectId);
		if (!object) {
...
export class QueryProvider {
	...
	constructor(queryEndpoint: string, versionCacheTimeoutInSeconds: number) {
		this.provider = new JsonRpcProvider(new Connection({ fullnode: queryEndpoint }), {
	...
	public async getOwnedObjects(
		...
		const { showType = true, showDisplay = true, showContent = true, showOwner = true } = options ?? {};
	...
	public async getOwnedObject(address: string, objectId: string): Promise<SuiObjectData | undefined> {
	...
	public async getDataOfObjects(objectIds: string[], options?: SuiObjectDataOptions) {
	...
	public async getGasBudgetFromDryRun(dryRunResult: DryRunTransactionBlockResponse): Promise<string> {
		...
		const refPrice = dryRunResult.input?.gasData.price ?? (await this.getReferenceGasPrice());
		const safeOverhead = QueryProvider.GAS_SAFE_OVERHEAD * BigInt(refPrice || 1n);
...
export class TxProvider {
	...
	static create(txEndpoint: string, versionCacheTimeoutInSeconds: number) {
		const provider = new JsonRpcProvider(new Connection({ fullnode: txEndpoint }), {
			// TODO: add socket options
			// socketOptions?: WebsocketClientOptions.
			versionCacheTimeoutInSeconds,
	...
	async transferObject(objectId: string, recipient: string, vault: Vault, gasBudgest?: number) {
	...
	public async signTransactionBlock(tx: TransactionBlock, vault: Vault): Promise<SignedTransaction> {
		const keypair = createKeypair(vault);
		const signer = new RawSigner(keypair, this.provider);
	 ...
	 * @param vault
	 */
	public async dryRunTransactionBlock(tx: TransactionBlock, vault: Vault): Promise<DryRunTransactionBlockResponse> {
		const keypair = createKeypair(vault);
		const signer = new RawSigner(keypair, this.provider);
