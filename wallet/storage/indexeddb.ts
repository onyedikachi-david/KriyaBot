...
import { DATA_VERSION, DB_NAME, DB_VERSION, GLOBAL_META_ID, StoreName } from "./constants";
...
export class IndexedDBStorage implements IStorage {
	...
	async getWallet(id: string): Promise<Wallet> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
					const request = db.transaction([StoreName.WALLETS]).objectStore(StoreName.WALLETS).get(id);
	...
	async getWallets(): Promise<Wallet[]> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
					const request = db.transaction([StoreName.WALLETS]).objectStore(StoreName.WALLETS).getAll();
	...
	async deleteAccount(walletId: string, accountId: string): Promise<void> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
					const transaction = db.transaction([StoreName.ACCOUNTS, StoreName.WALLETS], "readwrite");
					...
					getWalletRequest.onsuccess = (event) => {
						...
						if (typeof wallet === "undefined" || !wallet.accounts.find((ac) => ac.id === accountId)) {
	...
	async getAccounts(walletId: string): Promise<Account[]> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
					const transaction = db.transaction([StoreName.ACCOUNTS, StoreName.WALLETS], "readonly");
	...
	async getAccount(accountId: string): Promise<Account | null> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
					const request = db.transaction([StoreName.ACCOUNTS]).objectStore(StoreName.ACCOUNTS).get(accountId);
	...
	async updateMetaWalletsAndAccounts(meta: GlobalMeta, wallets: Wallet[], accounts: Account[]): Promise<void> {
		return await this.connection.then(
			async (db) =>
				await new Promise((resolve, reject) => {
	...
	private cleanupAccount(transaction: IDBTransaction, wallet: Wallet, accountId: string): void {
	...
	private async dbMigrationIfNeeded(db: IDBDatabase, newVersion: number) {
		...
		for (let currVer = oldVersion; currVer < newVersion; currVer++) {
			...
			if (!indexeddbMigrations.has(versionMigrationKey)) {
				return;
			}
			const migration = indexeddbMigrations.get(versionMigrationKey) as MigrationMethod;
			try {
